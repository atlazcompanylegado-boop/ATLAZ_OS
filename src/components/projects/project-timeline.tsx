...
/** Server Component — nunca mostra `kind` cru, UUID ou payload JSON (mesmo contrato da timeline de Clientes). */
export function ProjectTimeline({
  ...
  if (entries.length === 0) {
    return <EmptyState icon={History} title="Nenhuma atividade ainda" description="As mudanças neste projeto aparecerão aqui." />;
  }

  function buildHref(targetPage: number) {
    return `/projetos/${projectId}?tab=timeline&timelinePage=${targetPage}`;
  }
      ...
      <ol className="space-y-4 border-l border-border pl-5">
        {entries.map((entry) => (
          <li key={entry.id} className="relative">
            ...
            <p className="text-sm font-medium text-ink-1">{translateProjectEventKind(entry.kind)}</p>
