...
export interface TicketClientSelectorResult {
  rows: TicketClientOption[];
  total: number;
  page: number;
  pageSize: number;
}
 ...
 * carrega todos os clientes da org para montar um select (mesmo padrão de Projetos).
 * `onSearch` é a server action que consulta `listTicketClients`; a seleção final
 * ainda é revalidada no servidor pelo ticket-service ao salvar o formulário.
 */
export function TicketClientSelector({
  name,
  initialClient,
  onSearch,
  onChange,
  ...
  name: string;
  initialClient: TicketClientOption | null;
  onSearch: (q: string, page: number) => Promise<TicketClientSelectorResult>;
  onChange?: (client: TicketClientOption | null) => void;
  disabled?: boolean;
  error?: string;
}) {
  const [open, setOpen] = React.useState(false);
  const [selected, setSelected] = React.useState<TicketClientOption | null>(initialClient);
  const [q, setQ] = React.useState("");
  const [rows, setRows] = React.useState<TicketClientOption[]>([]);
  ...
  function choose(option: TicketClientOption) {
    setSelected(option);
    setOpen(false);
    onChange?.(option);
  }
